...
- **如何计算 (Calculation)**
  ...
  - \\(X_t = X\left[\ldots, 0 : D_t\right)\\)
  - \\(X_h = X\left[\ldots, D_t : D_t+D_h\right)\\)
  - \\(X_w = X\left[\ldots, D_t+D_h : D\right)\\)
...
- **Qwen2-VL 的 3D 卷积 (3D 处理)**：它使用了一个 **深度为 2 (Depth = 2)** 的 3D 卷积核。这意味着它一次性"吐揉"时间上相邻的**两帧**画面。它不再提取平面的 Patch，而是提取立体的 "3D Tube" (时空管)。这个 Tube 的维度是：\\(2 \times 14 \times 14\\)
...
# 3. Part III: [Qwen2.5-VL](https://arxiv.org/abs/2502.13923) —— 工程优化与训练范式的全面升级（2025）
...
\\(t_{abs}\\): 某一帧在视频中的真实时间（秒），\\(v\\): 帧率（例如每秒对应多少个 ID 单位）。
...
3. **编码器：**  这些 Token 进入 ViT，在大部分层中只在 \\(8 \times 8\\) 的 窗口内计算注意力（Window Attention），极低成本地提取特征，只在少数几层进行全局时空信息融合。
